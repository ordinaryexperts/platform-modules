# CLAUDE.md - Opsitron Modules Repository

This repository contains vetted OpenTofu/Terraform modules maintained by Ordinary Experts for use by [Opsitron](https://opsitron.com) clients.
...
│   │   ├── CHANGELOG.md      # Version history (maintained manually)
...
- `module.json` - Module metadata for Opsitron catalog
- `CHANGELOG.md` - Version history

### module.json

Every module MUST have a `module.json` that defines metadata synced to Opsitron:
...
Every module MUST maintain a `CHANGELOG.md` with an entry for each released version. Update the changelog **before** tagging a release.
...
### Added
- New `redis_enabled` variable for optional Redis cluster

### Changed
- Default instance type changed to `t3.medium`

### Fixed
- Security group rule allowing all egress
...
Rules:
- Use `## [x.y.z] - YYYY-MM-DD` headers (newest first)
- Use `### Added`, `### Changed`, `### Fixed`, `### Removed` subsections as needed
- One line per change, concise but descriptive
- Every version tag MUST have a corresponding changelog entry
...
2. A webhook to Opsitron that updates the module catalog (version, metadata from `module.json`, parsed variables/outputs)

Before tagging a release:
1. Update `CHANGELOG.md` in the module directory with the new version entry
2. Use conventional commit messages to help with release note generation:
